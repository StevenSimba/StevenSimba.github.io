...
var xValue = function(d) { return d.TOTAL_AREA_HA;}, 
    xScale = d3.scale.linear().range([0, width]), 
    xMap = function(d) { return xScale(xValue(d));}, 
    xAxis = d3.svg.axis().scale(xScale).orient("bottom");

 var yValue = function(d) { return d["FOREST_AREA"];},
    yScale = d3.scale.linear().range([height, 0]), 
    yMap = function(d) { return yScale(yValue(d));}, 
    yAxis = d3.svg.axis().scale(yScale).orient("left");

 var cValue = function(d) { return d.quartiles;},
...
d3.csv("js/one_countyForest.csv", function(error, data) {
	data.forEach(function(d) {
		d.FOREST_AREA = +d.FOREST_AREA;
		d.TOTAL_AREA_HA = +d.TOTAL_AREA_HA	})
...
//START LINE  CHART
d3.csv('js/forest_data.csv', function(error, data) {
    data.forEach(function (d) {
        d.year = +d.Year;
        d.Forestland = +d.Forestland;
    ...
    var chart = makeLineChart(data, 'Year', {
        'Forestland': {column: 'Forestland'},
        'Cropland': {column: 'Cropland'},
        'Settlement': {column: 'Settlement'}
    }, {xAxis: 'Years', yAxis: 'Land Use (ha)'});
...
function makeLineChart(dataset, xName, yObjs, axisLables) {
    ...
    chartObj.margin = {top: 15, right: 10, bottom: 30, left: 60};
    chartObj.width = 450 - chartObj.margin.left - chartObj.margin.right;
...
// Render the chart
    chartObj.render = function () {
        ...
        // Draw Axis
        chartObj.svg.append("g").attr("class", "x axis").attr("transform", "translate(0," + chartObj.height + ")").call(chartObj.xAxis).append("text").attr("class", "label").attr("x", chartObj.width / 2).attr("y", 25).style("text-anchor", "middle").text(chartObj.xAxisLable);

        chartObj.svg.append("g").attr("class", "y axis").call(chartObj.yAxis).append("text").attr("class", "label").attr("transform", "rotate(-90)").attr("y", 8).attr("x", -chartObj.height / 2).attr("dy", ".71em").style("text-anchor", "middle").text(chartObj.yAxisLable);
